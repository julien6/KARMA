...
""")
    ...
    def deploy_prometheus(self):
        """
        Déploie Prometheus dans le cluster à l'aide de Helm.
        """
        print("Deploying Prometheus with Helm...")
        try:
            # Ajouter le dépôt Helm pour Prometheus
            subprocess.run(["helm", "repo", "add", "prometheus-community",
                           "https://prometheus-community.github.io/helm-charts"], check=True)
            subprocess.run(["helm", "repo", "update"], check=True)

            # Installer Prometheus avec un timeout augmenté
            subprocess.run([
                "helm", "install", "prometheus", "prometheus-community/kube-prometheus-stack",
                "--namespace", self.cluster_name, "--create-namespace", "--timeout", "2m0s"
            ], check=True)
            print("Prometheus Helm chart deployed successfully.")

            # Attendre que tous les pods soient prêts
            print("Waiting for Prometheus pods to be ready...")
            subprocess.run([
                "kubectl", "wait", "--namespace", self.cluster_name,
                "--for=condition=ready", "pods", "--selector=release=prometheus", "--timeout=600s"
            ], check=True)
            print("All Prometheus pods are ready.")
    ...
    def create_cluster(self):
        ...
        try:
            ...
            # Étape 4 : Charger les images Docker dans le cluster
            ...
            # Étape 6 : Appliquer les manifests Kubernetes dans le namespace spécifique
            ...
            # Étape 7 : Vérifier les pods et services
            print("Checking cluster status...")
            subprocess.run(["kubectl", "get", "pods",
                           "--namespace", self.cluster_name], check=True)
    ...
    def validate_prometheus(self):
        """
        Vérifie que Prometheus collecte les métriques des mocks en lançant kubectl port-forward.
        """
        print("Validating Prometheus setup...")
        try:
            # Commande de port-forward en arrière-plan
            print("Starting kubectl port-forward for Prometheus...")
            process = Popen(
                ["kubectl", "port-forward", "-n", self.cluster_name,
                    "svc/prometheus-kube-prometheus-prometheus", "9090:9090"],
                stdout=DEVNULL,  # Redirige la sortie standard
                stderr=DEVNULL,  # Redirige les erreurs
                preexec_fn=os.setpgrp  # Détache du processus principal
            )
            print("Prometheus is accessible at http://localhost:9090")
            print(f"Port-forward process PID: {process.pid}")

            self.prometheus_pid = process.pid

        except Exception as e:
            print(f"Failed to validate Prometheus: {e}")
    ...
    def wait_for_pods_ready(self, timeout=300):
        ...
        """
        print(
            f"Waiting for all pods to be ready in namespace '{self.cluster_name}'...")
        for i in range(timeout // 5):
            ...
            if not result.stdout.strip():
                print(f"Iteration {i+1}: All pods are ready.")
                return
            else:
                print(
                    f"Iteration {i+1}: Pods are still initializing:\n{result.stdout.strip()}")
            time.sleep(5)
    ...
    def delete_cluster(self):
        ...
        print("Killing Prometheus redirection process...")
        try:
            os.kill(self.prometheus_pid, signal.SIGTERM)
            print(f"Sent SIGTERM signal to process {self.prometheus_pid}")
        except OSError:
            print(
                f"Failed to send SIGTERM signal to process {self.prometheus_pid}")
        finally:
            self.prometheus_pid = None


def generate_and_deploy_cluster(topology_path: str):
    """
    Génère et déploie le cluster depuis le début.
    """
    kc = KarmaCluster(topology_path)
    try:
